...
const EditFoxcomOriginals = () => {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pagination.page, searchTerm, loadingSection]);

  const loadSection = async () => {
    try {
      setLoadingSection(true);
      setError("");
      const data = await foxcomOriginalsAPI.getById(id);
      const section = data.originals;

      if (section) {
        setFormData({
          title: section.title || "FOXECOM Originals",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await foxcomOriginalsAPI.update(id, {
        title: formData.title,
        isActive: formData.isActive,
        productIds: selectedProducts,
      });

      setSuccess("FOXECOM Originals section updated successfully!");
  ...
  return (
    <div className="container-fluid p-3 p-md-4">
      <div className="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-2">
        <h4 className="mb-0 fw-bold">Edit FOXECOM Originals</h4>
        <button
          type="button"
      ...
      <form onSubmit={handleSubmit}>
                ...
                <div className="mb-3">
                  <div className="form-check form-switch">
                    <input
                      ...
                      type="checkbox"
                      id="isActiveOriginalsEdit"
                      checked={formData.isActive}
                      onChange={(e) => setFormData((prev) => ({ ...prev, isActive: e.target.checked }))}
                    />
                    <label className="form-check-label" htmlFor="isActiveOriginalsEdit">
                  ...
                  <>
                    <div className="mb-2">
                      <div className="form-check">
                        <input
                          className="form-check-input"
                          type="checkbox"
                          id="selectAllOriginalsEdit"
                          checked={
                            products.length > 0 && products.every((p) => selectedProducts.includes(p.id))
                          }
                          onChange={handleSelectAll}
                        />
                        <label className="form-check-label" htmlFor="selectAllOriginalsEdit">
                    ...
                    >
                      {products.map((product) => (
                        <div key={product.id} className="d-flex align-items-center gap-2 p-2 border-bottom">
                          <input
                            type="checkbox"
                            className="form-check-input"
                          ...
                          <div className="flex-grow-1">
                            ...
                            <small className="text-muted">
                              ...
                              {product.discountPrice && ` (Rs. ${parseFloat(product.discountPrice).toFixed(2)})`}
                            </small>
                          </div>
                        </div>
                      ))}
        ...
        <div className="d-flex flex-wrap gap-2 justify-content-end">
          <button type="button" className="btn btn-outline-secondary" onClick={() => navigate("/admin/foxcom-originals")}>
            Cancel
          </button>
...
export default EditFoxcomOriginals;
