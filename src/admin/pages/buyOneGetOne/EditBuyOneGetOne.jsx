...
import { adminAPI, buyOneGetOneAPI, getImageUrl } from "../../../utils/api";

const EditBuyOneGetOne = () => {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pagination.page, searchTerm, loadingDeal]);

  const loadSection = async () => {
    try {
      setLoadingDeal(true);
      const data = await buyOneGetOneAPI.getById(id);
      const deal = data.deal;

      if (deal) {
        setFormData({
          title: deal.title || "Buy One Get One",
      ...
      console.error(err);
      setError(err.message || "Failed to load Buy One Get One section");
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await buyOneGetOneAPI.update(id, {
      ...
      setSuccess("Buy One Get One section updated successfully!");
      setTimeout(() => {
        navigate("/admin/buy-one-get-one");
      }, 1500);
    } catch (err) {
      console.error(err);
      setError(err.message || "Failed to update Buy One Get One section");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="container-fluid p-3 p-md-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="mb-0 fw-bold">Edit Buy One Get One</h4>
        <button
          type="button"
          className="btn btn-outline-secondary"
          onClick={() => navigate("/admin/buy-one-get-one")}
        >
          Back
        </button>
      </div>
                  ...
                  <>
                    <div className="mb-2">
                      <div className="form-check">
                        ...
                        />
                        <label className="form-check-label" htmlFor="selectAllBogoEdit">
...
export default EditBuyOneGetOne;
